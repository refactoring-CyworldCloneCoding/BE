...
import { GuestBooks } from '../db/repositories';
...
class GuestBooksService {
  ...
  createBook = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const existUser = GuestBooks.findByUser(userId);
  ...
  getBooks = async (req: Request, res: Response, next: NextFunction) => {
    const { userId } = req.params;
    const books = GuestBooks.getBooks(userId);
  ...
  deleteBook = async (req: Request, res: Response, next: NextFunction) => {
    ...
    GuestBooks.deleteBook(guestbookId);
  };
}
